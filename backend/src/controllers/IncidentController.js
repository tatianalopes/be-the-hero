...
module.exports = {
    ...
    async create(request, response) {
        ...
        const [id] = await dbConnection('incidents').insert({ title, description, value, ong_id });
    ...
    async delete(request, response) {
        ...
        if (incident.ong_id !== ong_id) {
            return response.status(401).json({ error: 'Operation not permitted.' }); // unauthorized operation
        }
